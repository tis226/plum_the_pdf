...
def line_bounds_toporigin(ln):
    """Return (top, bottom) in top-origin coords for the provided line."""

    tops = [w.get("top") for w in ln if w.get("top") is not None]
    bottoms = [w.get("bottom") for w in ln if w.get("bottom") is not None]
...
def chunk_column_by_margin(page, bbox, left_margin_x: Optional[float], tol: float,
    ...
    cur: List[List[Dict[str, Any]]] = []
    state = {"has_option": False, "last_bottom": None}
    ...
    def flush():
        ...
        if text:
            chunk_bbox = lines_to_bbox(cur, float(page.height)) or bbox
            chunk = {"text": text}
            if chunk_bbox is not None:
                chunk["bbox"] = tuple(float(v) for v in chunk_bbox)
            chunks.append(chunk)
        cur.clear()
        state["has_option"] = False
        state["last_bottom"] = None

    gap_threshold = max(4.5, float(y_tol) * 1.6)
    ...
    for ln in lines:
        ...
        norm_line = norm_space(text)
        is_option_line = bool(stripped) and stripped[0] in CIRCLED
        is_new_question_line = bool(QUESTION_NUM_RE.match(norm_line))
        x0 = line_x0(ln)
        line_top, line_bottom = line_bounds_toporigin(ln)
        prev_bottom = state.get("last_bottom")
        gap = (line_top - prev_bottom) if (line_top is not None and prev_bottom is not None) else 0.0

        margin_hit = abs(x0 - left_margin_x) <= tol
        should_start_new = False
        if cur:
            if margin_hit and not is_option_line:
                if is_new_question_line:
                    should_start_new = True
                elif gap >= gap_threshold and not state.get("has_option"):
                    should_start_new = True
                elif gap >= gap_threshold * 1.5:
                    should_start_new = True
            elif gap >= gap_threshold * 2.0 and not is_option_line and margin_hit:
                should_start_new = True
